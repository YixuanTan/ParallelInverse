...
// The Sensitity Matrix mentioned in the inverse paper is actually R_matrix * S_matrix in this code.
// In another words, S_matrix is the fully sensitivity matrix of all d.o.f.s with respect to the heat generation.
...
double Constants::lambda_ = 1.0e-11; //0;//1.0e-10; //5.0e-11;//1.0e-11;//2.5e-12; //1e-11; //5e-12; //0;//;
...
public:
    ...
    std::vector<double>& get_current_in_heater()
    {return currents_in_heater_;}
    double get_temperature_on_sample_left_end() const
    {return temperature_on_sample_left_end_;}
    double get_temperature_on_sample_right_end() const
    {return temperature_on_sample_right_end_;}
...
private:
    ...
    double maximum_temperature_change_per_time_increment_;
    std::vector<double> currents_in_heater_;
    double temperature_on_sample_left_end_;
    double temperature_on_sample_right_end_;
};
void ReadInput::ScanInputInformation(){
    ...
    ifs>>ignore_this_string;
    currents_in_heater_.resize(Constants::kNumOfHeaters_, 0.0);
    ifs>>temperature_on_sample_left_end_;
    ifs>>ignore_this_string;
    ifs>>temperature_on_sample_right_end_;
    ifs>>ignore_this_string;
    ifs.close();
}
...
public:
    ...
    {return heater_crosssection_area_mm_square_;}
    double get_resistivity(double);
    double get_resistivity_derivative(double);
...
double TemperatureDependentVariables::get_resistivity(const double temperature){
    double resistivity;
    resistivity = -6.674e-13*temperature*temperature + 2.462e-9*temperature - 1.893e-7;// resistivity used in the IPISE paper.
    return resistivity;
}

double TemperatureDependentVariables::get_resistivity_derivative(const double temperature){
...
//    resistivity = 1.403e-15*temperature*temperature + 1.037e-8*temperature - 2.831e-6;// titanium heater
    resistivity = get_resistivity(temperature);// resistivity used in the IPISE paper.
...
double TemperatureDependentVariables::get_body_heat_flux_derivative(const double temperature, const double current_in_element_ma){ //Cal/g/K = 4.184e9 mJ/tonne/K
    ...
    double heater_crosssection_area_m_square=heater_crosssection_area_mm_square_*1.0e-3*1.0e-3;  //mm^2->m^2
    resistivity_derivative = get_resistivity_derivative(temperature);// titanium heater
...
public:
    ...
    void HeatSupply(int, int, PETSC_STRUCT*, std::vector<int>&, std::vector<double>&, Initialization*);
...
private:
    ...
    std::vector<double> element_heat_vector_;
};
...
void HeaterElements::HeatSupply(const int element_number, const int heater_element_number, PETSC_STRUCT* obj, std::vector<int>&equation_numbers_in_elements, std::vector<double>&current_heat_generation, Initialization* const initialization){//Calculate internal load contribution
    //integration rule
        ...
        for(int l=0;l<num_of_integration_points;l++){
            ...
            for(int j=0;j<Constants::kNumOfNodesInElement_;j++){
                ...
                if(equation_number>=0){
                    PetscScalar add_term = shape_function_[j]*current_heat_generation[heater_number]*determinant_of_jacobian_matrix_*ksi_weight*eta_weight;
...
void HeaterElements::set_element_heat_matrix(){
    /*  //----------------integration rule------------------
    ...
    for(int i=0;i<Constants::kNumOfNodesInElement_;i++){
        for(int k=0;k<num_of_integration_points;k++){
            ...
            for(int l=0;l<num_of_integration_points;l++){
                ...
                element_heat_vector_[i] += shape_function_[i]*determinant_of_jacobian_matrix_*ksi_weight*eta_weight;;
            }
        }
    }
}

void HeaterElements::MapElementalToGlobalHeatMatrix(PETSC_STRUCT* obj, std::vector<int>&equation_numbers_in_elements, const int element_number, const int heater_number){
    for(int i=0;i<Constants::kNumOfNodesInElement_;i++){
        int equation_number=equation_numbers_in_elements[i+element_number*Constants::kNumOfNodesInElement_];
        if(equation_number>=0){
            PetscErrorCode ierr = MatSetValue(obj->heat_matrix, (PetscInt)equation_number, (PetscInt)heater_number, (PetscScalar)(element_heat_vector_[i]), ADD_VALUES);
        }
    }
    //  PrintHeatMatrix(heat_matrix);
}
...
void MaterialParameters::set_material_id_of_elements(Initialization *const initialization){
    ...
    *((*((*initialization).get_mesh_parameters())).get_dimensions_of_x()-1)-1;
    ...
    int first_element_of_heater_gap_mixed = (Constants::kMeshSeedsAlongSiliconThickness_+(*((*initialization).get_mesh_parameters())).get_mesh_seeds_along_isolater_thickness())*((*((*initialization).get_mesh_parameters())).get_dimensions_of_x()-1) + (*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_end();
    int last_element_of_heater_gap_mixed = (Constants::kMeshSeedsAlongSiliconThickness_+(*((*initialization).get_mesh_parameters())).get_mesh_seeds_along_isolater_thickness()+1)*((*((*initialization).get_mesh_parameters())).get_dimensions_of_x()-1) -1 - (*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_end();
    int first_element_of_pure_layer_of_silicondioxide = (Constants::kMeshSeedsAlongSiliconThickness_+(*((*initialization).get_mesh_parameters())).get_mesh_seeds_along_isolater_thickness()+1)*((*((*initialization).get_mesh_parameters())).get_dimensions_of_x()-1);
    int last_element_of_pure_layer_of_silicondioxide = (Constants::kMeshSeedsAlongSiliconThickness_+(*((*initialization).get_mesh_parameters())).get_mesh_seeds_along_isolater_thickness()+1 + (*((*initialization).get_mesh_parameters())).get_mesh_seeds_along_silicondioxide_thickness())*((*((*initialization).get_mesh_parameters())).get_dimensions_of_x()-1)-1;
...
void OutputResults::OutputCopperSurfaceTemperature(Initialization *const initialization, GenerateMesh *const generate_mesh, std::vector<double>& current_temperature_field){
    ...
    /*
    char buffer[30];
    sprintf(buffer, "SurfaceTemperature_%.3f.txt", peakPos);
    char *filename = buffer; 
...
class CopperSurfTemperatureDistribution{
public:
    ...
    std::vector<double> &get_temperature_on_sample_surface()
    {return temperature_on_sample_surface_;}
    void set_elements_on_sample_surface(int);
    void set_equations_on_sample_surface(std::vector<int> &);
    std::vector<int>& get_elements_on_sample_surface()
    {return elements_on_sample_surface_;}
    std::vector<int>& get_equations_on_sample_surface()
    {return equations_on_sample_surface_;}
    int get_num_of_elements_on_sample_surface() const
    {return num_of_elements_on_sample_surface_;}
    int get_first_element_on_sample_surface() const
    {return first_element_on_sample_surface_;}
    int get_first_equation_on_sample_surface() const
    {return first_equation_on_sample_surface_;}
    int get_first_node_on_sample_surface() const
    {return first_node_on_sample_surface_;}
    
private:
    int num_of_elements_on_sample_surface_;
    int num_of_equations_on_sample_surface_;
    int first_equation_on_sample_surface_;
    int first_node_on_sample_surface_;
    int first_element_on_sample_surface_;
    std::vector<double> temperature_on_sample_surface_;
    std::vector<int> elements_on_sample_surface_;
    std::vector<int> equations_on_sample_surface_;
};
void CopperSurfTemperatureDistribution::InitializeCopperSurfTemperatureDistribution(int num_of_equations, int num_of_nodes, int num_of_elements, Initialization* initialization){
    num_of_elements_on_sample_surface_ = Constants::kNumOfHeaters_*((*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_heater()+
                                                                    (*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_gap());
    num_of_equations_on_sample_surface_ = num_of_elements_on_sample_surface_ + 1;
    first_element_on_sample_surface_ = num_of_elements-(*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_end()
    -num_of_elements_on_sample_surface_;
    first_equation_on_sample_surface_ = num_of_equations-(*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_end()-
    num_of_equations_on_sample_surface_; // 0 indexed
    first_node_on_sample_surface_ = num_of_nodes-(*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_end()-
    num_of_equations_on_sample_surface_; // 0 indexed
    elements_on_sample_surface_.resize(num_of_elements_on_sample_surface_);
...
void CopperSurfTemperatureDistribution::set_elements_on_sample_surface(const int num_of_elements){
    for(int i=0; i<num_of_elements_on_sample_surface_;i++){
        elements_on_sample_surface_[i] = (first_element_on_sample_surface_ + i);
    }
}

void CopperSurfTemperatureDistribution::set_equations_on_sample_surface(std::vector<int> &equation_numbers_in_elements){
    // form equations_on_sample_surface----------------------------------*/
    for(int l=0; l<num_of_elements_on_sample_surface_; l++){
        for(int k=Constants::kNumOfNodesInElement_-1; k>1; k--){
            int equation_number = equation_numbers_in_elements[Constants::kNumOfNodesInElement_*elements_on_sample_surface_[l]+k]; //3rd and 4th equation in element
            bool add_this_equation = true;
            if(k==Constants::kNumOfNodesInElement_-1){// only 4th equation number in element may be repeatedly recorded.
                for(int ii=0;ii<equations_on_sample_surface_.size();ii++){
                    if(equation_number==equations_on_sample_surface_[ii]){
                        add_this_equation = false;
                        break;//break for int ii
                    }
                }
            }
            if(add_this_equation == true){
                equations_on_sample_surface_.push_back(equation_number);
            }
        }
    }
    /*------------------------------form equations_on_sample_surface----------------------------------*/
...
//first to get the node ID of those on the copper surface for generating the R matrix
int CopperSurfTemperatureDistribution::RCoefficientMatrix(int i, int j){ // num_of_equations_on_sample_surface by num_of_equations
    if(i>=num_of_equations_on_sample_surface_){
        printf("out of the R matrix index domain\n");
        exit(-1);
    }
    if(j>=first_equation_on_sample_surface_ && j<=first_equation_on_sample_surface_+num_of_equations_on_sample_surface_-1
       && i==(j-first_equation_on_sample_surface_) ){ //first_equation_on_sample_surface is 0 indexed
        return 1;
    }
    else
        return 0;
...
void CopperSurfTemperatureDistribution::PrintOutTemperature(int num_of_equations){
    for(int i=0;i<temperature_on_sample_surface_.size();i++){
        printf("temperature_on_sample_surface_[%d]=%f\n",i,temperature_on_sample_surface_[i]);
    }
    printf("\n\n R matrix is:\n");
    for(int i=0;i<num_of_equations_on_sample_surface_;i++){
        for(int j=0;j<num_of_equations;j++){
            printf("%d    ", RCoefficientMatrix(i, j));
        }
        printf("\n");
    }
}
...
private:
    InverseVariables inverse_variables_;
    double error_norm_last_;
    double error_norm_current_;
...
void InverseAnalysisMatrices::InitializeInverseAnalysisMatrices(const int num_of_equations, const int num_of_equations_on_sample_surface, CopperSurfTemperatureDistribution *copper_surface_temperature_distribution, PETSC_STRUCT* obj){
    ...
    for(int ii=0;ii<num_of_equations_on_sample_surface;ii++) {
        for(int jj=0;jj<num_of_equations;jj++) {
            ...
            ierr = MatSetValue(obj->R_matrix, (PetscInt)ii, (PetscInt)jj, add_term, ADD_VALUES);
        }
    }
...
bool InverseAnalysisMatrices::HeatGenerationSolver(PETSC_STRUCT* obj, char *Sname, double start_pos, double end_pos){
    ...
    if( error_norm_last_ != 0.0 && (error_norm_current_/error_norm_last_ > Constants::kCutoff_) && (error_norm_current_/error_norm_last_ <= 1.01)){
    ...
    if((error_norm_current_ < Constants::kTemperatureTolerance_ || count_equal_norm_time_ >= 5) && (error_norm_last_ != 0.0 && error_norm_current_/error_norm_last_ > Constants::kCutoff_ && error_norm_current_/error_norm_last_ <= 1.01 )){
        ...
        PETSC_VEC extra_regularization;

        ierr = VecCreate(PETSC_COMM_WORLD, &extra_regularization); //CHKERRQ(ierr);
        ierr = VecSetSizes(extra_regularization, PETSC_DECIDE, Constants::kNumOfHeaters_); //CHKERRQ(ierr);
        ierr = VecSetFromOptions(extra_regularization); //CHKERRQ(ierr);

        for(int heater = 0; heater < Constants::kNumOfHeaters_; heater++) {
            ...
            if(heater != 0 && heater_pos > (start_pos + end_pos) / 2) { 
                double extreme = (Constants::lambda_ == 0.0 ? 1.0e-11 : 1.0e-8 * Constants::lambda_); // add an extremely large regularization factor to prevent increasing heat on the rhs heaters
                ierr = VecSetValue(extra_regularization, (PetscInt)heater, (PetscScalar)extreme, INSERT_VALUES);
            }/*else (heater !=0 && heater_pos + periodLen < start_pos) {
                double extreme = Constants::lambda_ == 0.0 ? 1.0e-11 : 1.0e-9 * Constants::lambda_; // add an large regularization factor to prevent increasing heat on the lhs heaters
                ierr = VecSetValue(extra_regularization, (PetscInt)heater, (PetscScalar)extreme, INSERT_VALUES);
            } else {
        ...
        for (int k = 0; k < num_of_surfacenodes; k++){
            ierr = VecSetValue(temperature_increment, (PetscInt)k, (PetscScalar)((inverse_variables_.temperature_increment_)[k]), ADD_VALUES);
        }
        ...
        ierr = VecDestroy(extra_regularization); //CHKERRQ(ierr);

        PetscViewer viewer;
	//	ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD, Sname, &viewer); //CHKERRQ(ierr);                                                                                                         
	//ierr = PetscViewerPushFormat(viewer,PETSC_VIEWER_ASCII_MATLAB); //CHKERRQ(ierr);                                                                                                        
	//ierr = PetscObjectSetName((PetscObject)RS_matrix,"S_matrix"); //CHKERRQ(ierr);                                                                                                          
	//ierr = PetscObjectSetName((PetscObject)((*obj).rhs_inverse),"rhs"); //CHKERRQ(ierr);                                                                                                    
	//ierr = MatView(RS_matrix,viewer); //CHKERRQ(ierr);                                                                                                                                      
	//ierr = VecView((*obj).rhs_inverse, viewer); //CHKERRQ(ierr);                                                                                                                                    
...
void InverseAnalysisMatrices::UpdateCurrents(const double heater_cross_section_area_si_unit, Initialization *initialization, HeaterElements *heater_elements, std::vector<int> &elements_as_heater, std::vector<int> &nodes_in_elements, std::vector<double> &x_coordinates, std::vector<double> &y_coordinates, std::vector<double> &current_temperature_field, TemperatureDependentVariables *temperature_dependent_variables){
    ...
    for(int heater_number=0; heater_number<Constants::kNumOfHeaters_; heater_number++){
        ...
        (*(*initialization).get_currents_in_heater()).get_current_in_heater()[heater_number] = current_si_unit*1.0e3; //A -> mA
    }
}
...
//    MPI_Comm_size (MPI_COMM_WORLD, &size);	/* get number of processes */
    ...
    */
    ...
    /*-----------------------------inverse--------------------------------*/
    CopperSurfTemperatureDistribution copper_surface_temperature_distribution;
    ...
    std::vector<int>& elements_on_sample_surface = copper_surface_temperature_distribution.get_elements_on_sample_surface();
    std::vector<int>& equations_on_sample_surface = copper_surface_temperature_distribution.get_equations_on_sample_surface();
    int num_of_equations_on_sample_surface = copper_surface_temperature_distribution.get_num_of_equations_on_sample_surface();
    int num_of_elements_on_sample_surface = copper_surface_temperature_distribution.get_num_of_elements_on_sample_surface();
    int first_element_on_sample_surface = copper_surface_temperature_distribution.get_first_element_on_sample_surface();
...
//    Assemble assemble;
    ...
    //create matrices and vectors for inverse analysis
    ...
    while(1){ // the inverse iteration loop
        ...
        char bfer[30];
        sprintf(bfer, "Sname%d.m",inverse_iteration);
        char *Sname = bfer;
	...
	std::ofstream out;
    out.open("current_densities.txt", std::ios::app);
    ...
    for(int i=0;i<Constants::kNumOfHeaters_;i++){
        out << (*(initialization.get_currents_in_heater())).get_current_in_heater()[i] << "\n";
    }
    out.close();
